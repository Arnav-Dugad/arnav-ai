...
function applyTheme(name){
  ...
  ['--bg','--bg1','--bg2','--bg3'].forEach(p=>r.style.removeProperty(p));
...
function renderHistory(list){
  const c=$('hist-today');
  c.innerHTML='<div class="hist-label">Recent</div>';
  if(!list.length){
    c.innerHTML+='<div style="padding:8px 10px;font-size:12px;color:var(--text3);">No conversations yet</div>';
    return;
  }
  // Pinned first, then rest
  const sorted=[...list].sort((a,b)=>(b.pinned||0)-(a.pinned||0));
  sorted.forEach(item=>{
    const d=document.createElement('div');
    d.className='hist-item'+(item.id===currentChatId?' on':'')+(item.pinned?' pinned':'');
    d.dataset.id=item.id;
    const pinIcon=item.pinned?'<svg class="pin-icon" fill="currentColor" viewBox="0 0 24 24" width="10" height="10"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>':'';
    ...
    d.addEventListener('click',e=>{if(!e.target.closest('.hist-actions')&&!e.target.closest('.hist-item-text[contenteditable="true"]'))loadChat(item.id);});
    c.appendChild(d);
  });
}
...
function startRename(e,id){
  ...
  const finish=async()=>{
    ...
    if(newTitle!==orig){
      ...
      await saveConversation(id,false);
      toast('Renamed','info',1500);
    }
  };
...
async function togglePin(e,id){
  ...
  if(currentUserId&&window._db){
    try{
      const ref=window._fsDoc(window._db,'users',currentUserId,'conversations',id);
      await window._fsSet(ref,{pinned:h.pinned},{merge:true});
    }catch(err){}
  }
  toast(h.pinned?'Conversation pinned':'Unpinned','info',1500);
}
...
async function _doClearAll(){
  if(currentUserId&&window._db){
    try{
      await Promise.all(allHistory.map(h=>window._fsDel(window._fsDoc(window._db,'users',currentUserId,'conversations',h.id))));
    }catch(e){}
  }
  allHistory=[];sessions={};msgs=[];currentChatId=null;chatTitle='';
  bookmarks=[];bookmarksSet=new Set();
  try{localStorage.removeItem('arnav-history');}catch(e){}
  renderHistory([]);
  newChat();
  toast('All conversations cleared','info');
}
...
function scrollToBottom(){const a=$('msgs-area');a.scrollTo({top:a.scrollHeight,behavior:'smooth'});}
...
function fmt(raw){
  ...
  t=t.replace(/```(\w*)\n?([\s\S]*?)```/g,(m,lang,code)=>{
    ...
    return `<div class="code-block cb-${esc(l)}"><div class="code-header"><span class="code-lang-wrap"><span class="lang-dot"></span><span class="code-lang">${esc(l)}</span><span class="code-line-count">${lineCount} line${lineCount!==1?'s':''}</span></span><div class="code-header-actions">${expandBtn}${copyBtn}</div></div><pre data-linecount="${lineCount}"><code class="hljs">${highlighted}</code></pre></div>`;
  });
  // inline code
  t=t.replace(/`([^`\n]+)`/g,'<code>$1</code>');
  // headings
...
function openFullscreen(btn){
  ...
  const fsDot=$('fs-lang-dot');
  if(fsDot&&langDot)fsDot.className=langDot.className;
  const fsLang=$('fs-lang');if(fsLang)fsLang.textContent=lang;
  const fsCode=$('fs-code');if(fsCode){fsCode.innerHTML=code.innerHTML;fsCode.className=code.className;}
  $('fullscreen-modal').classList.add('on');
  // Apply line numbers in fullscreen
  if(settings.lines&&fsCode){
    const pre=fsCode.closest('pre')||fsCode.parentElement;
...
function appendAIBubble(text,webUsed,msgIndex,elapsed){
  ...
  <div class="ai-body-wrap">
    <div class="ai-meta">
      ...
      <span class="ai-time">${tStr()}</span>${wb}${timeBadge}${wcBadge}
    </div>
...
async function callAPI(text,isNew){
  ...
  try{
    ...
    appendAIBubble(reply,webUsed,msgs.length-1,elapsed);
...
function toggleVoice(){
  ...
  _recog=new SR();
  _recog.lang='en-US';_recog.continuous=false;_recog.interimResults=true;_recog.maxAlternatives=1;
  _recog.onstart=()=>{
    _voiceOn=true;
  ...
  _recog.onresult=ev=>{
    ...
    $('cinput').value=txt.trim();onInput($('cinput'));
  };
  _recog.onend=()=>{
  ...
  _recog.onerror=ev=>{
    ...
    $('input-box').classList.remove('listening');
    $('cinput').placeholder='Message Arnav AI…';
    if(ev.error!=='no-speech'&&ev.error!=='aborted')toast('Mic error: '+ev.error,'err');
  };
...
function _startTts(rawText,btn){
  ...
  const clean=rawText
    ...
    .replace(/[🌐⚠️✅❌💡🔍✍️🧠🎤]/g,'').replace(/\s+/g,' ').trim().slice(0,3000);
  if(!clean)return;
  const utt=new SpeechSynthesisUtterance(clean);
  utt.lang='en-US';utt.rate=0.92;utt.pitch=1.0;utt.volume=1.0;
  const _go=()=>{
    const v=_getUSVoice();if(v)utt.voice=v;
    if(btn){
      _ttsBtn=btn;btn.classList.add('speaking');
    ...
    utt.onend=()=>{_clearTtsBtn();};utt.onerror=()=>{_clearTtsBtn();};
...
function doShare(type){
  ...
  if(type==='copy'){
    navigator.clipboard.writeText(text).then(()=>{toast('Copied to clipboard!','ok');closeModalId('share-modal');}).catch(()=>toast('Copy failed — try download instead','err'));
    ...
    if(navigator.share){navigator.share({title:chatTitle||'Arnav AI Chat',text}).catch(()=>{});}
    else{navigator.clipboard.writeText(text).then(()=>toast('Copied (native share unavailable)','info'));}
...
function cancelEditMsg(idx){
  ...
  bubble.innerHTML=renderUserBubbleHtml(msgs[idx]?.content||'',idx);
}
...
// ══════════════════════════════════════
// USAGE STATS — Firestore + local fallback
// ══════════════════════════════════════
...
async function openStats(){
  closeUserMenu();
  // Show modal immediately with local estimates
  const localMsgs=allHistory.reduce((sum,h)=>sum+(sessions[h.id]?.msgs?.filter(m=>m.role==='user').length||0),0);
  $('stat-msgs').textContent=localMsgs||'—';
  $('stat-convs').textContent=allHistory.length||'—';
  ...
  // Then fetch real Firestore stats
  if(currentUserId&&window._db){
    try{
      const ref=window._fsDoc(window._db,'users',currentUserId,'stats');
      ...
      if(snap.exists()){
        ...
        if(data.totalMessages!=null)$('stat-msgs').textContent=data.totalMessages;
        if(data.totalConversations!=null)$('stat-convs').textContent=data.totalConversations;
      }
    }catch(e){}
  }
}
...
function openBookmarks(){
  ...
  if(!bookmarks.length){
    list.innerHTML='<div class="bm-empty">No bookmarks yet.<br>Tap the bookmark icon on any AI message to save it.</div>';
  }else{
...
async function removeBookmark(e,bmId,convId,msgIdx){
  ...
  if(!$('bm-list').children.length){
    $('bm-list').innerHTML='<div class="bm-empty">No bookmarks yet.<br>Tap the bookmark icon on any AI message to save it.</div>';
  }
  toast('Bookmark removed','info');
  if(currentUserId&&window._db){
